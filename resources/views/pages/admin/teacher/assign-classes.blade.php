...
<form action="/admin/teachers/{{$teacher->id}}/assign-class" method="post"
    class="shadow-lg p-3 mb-5 mt-3 bg-body-tertiary rounded">
    @csrf
    <div class="mb-3">
        <label for="teacher" class="form-label">مدرس</label>
        <input type="text" name="teacher" id="teacher" class="form-control"
            value="{{ $teacher->first_name }} {{ $teacher->first_name }}" readonly>
        <x-form-error name="teacher" />
    </div>
    <div class="mb-3">
        <label for="class" class="form-label">الفصول الدراسية</label>
        <div class="row">
